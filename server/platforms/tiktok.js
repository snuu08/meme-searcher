...
function mapVideo(item) {
  ...
  return {
    ...
    authorId: item.username || item.author || item.display_name || null,
    country: country,
...
async function queryVideos(token, query, period, now) {
  var start = Math.floor(periodStart(period, now).getTime() / 1000);
  var end = Math.floor(now / 1000);
  var res = await fetch("https://open.tiktokapis.com/v2/research/video/query/?fields=id,video_description,create_time,username,region_code,view_count,like_count,comment_count,share_count", {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      query: {
        and: [{ field_name: "keyword", operation: "IN", field_values: [query] }]
      },
...
async function fetchPosts(options) {
  ...
  try {
    ...
    var jobs = queries.slice(0, TREND_CONFIG.limits.tiktokMaxQueries).map(function (q) {
      return queryVideos(token, q, period, now);
    });
